...
class Trainer(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)  # Removed `default=datetime.now`

    def __str__(self):
        return f"Trainer: {self.first_name} {self.last_name}"

    def get_average_rating(self):
        """Get the average rating of the trainer based on the related feedbacks."""

        avg_rating = self.feedbacks.aggregate(Avg('rating'))  # Aggregate the average rating of all feedbacks
        return avg_rating['rating__avg'] if avg_rating['rating__avg'] else 0.0  # Return 0.0 if no feedbacks
...
class TrainerFeedback(models.Model):
    ...
    def __str__(self):
        ...
        class Meta:
            ordering = ['-created_at']  # Ordering feedback by created_at, newest first
            indexes = [
                models.Index(fields=['trainer', 'created_at'])  # Index feedback by trainer and date
        ]